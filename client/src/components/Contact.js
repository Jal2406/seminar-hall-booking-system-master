import React from "react";
const Contact = () => {
  return (
    <div className="flex min-h-screen w-full flex-wrap items-center justify-center m-4">
      <div className="w-full rounded-xl p-12 shadow-2xl shadow-blue-200 mx-auto md:w-8/12 lg:w-5/12 bg-white">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="col-span-1 lg:col-span-9">
            <div className="text-center lg:text-left">
              <h2 className="text-2xl font-bold text-zinc-700">
                Prof. Hitesh Makwana
              </h2>
              <p className="mt-2 text-l font-semibold text-zinc-700">
                Faculty Coordinator
              </p>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8 text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Email</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">
                  hiteshmakwana@charusat.ac.in
                </p>
              </div>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8  text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Phone</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">9827253854</p>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="w-full rounded-xl p-12 shadow-2xl mx-auto shadow-blue-200 md:w-8/12 lg:w-5/12 bg-white">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="col-span-1 lg:col-span-9">
            <div className="text-center lg:text-left">
              <h2 className="text-2xl font-bold text-zinc-700">
                Jal Mayani
              </h2>
              <p className="mt-2 text-l font-semibold text-zinc-700">
                Studenet Coordinator
              </p>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8 text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Email</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">
                  22dit033@charusat.edu.in
                </p>
              </div>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8  text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Phone</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">7041340328</p>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="w-full rounded-xl p-12 shadow-2xl mx-auto shadow-blue-200 md:w-8/12 lg:w-5/12 bg-white">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="col-span-1 lg:col-span-9">
            <div className="text-center lg:text-left">
              <h2 className="text-2xl font-bold text-zinc-700">
                Vedant Bhatia
              </h2>
              <p className="mt-2 text-l font-semibold text-zinc-700">
                Studenet Coordinator
              </p>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8 text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Email</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">
                  22dit004@charusat.edu.in
                </p>
              </div>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8  text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Phone</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">9714269527</p>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="w-full rounded-xl p-12 shadow-2xl mx-auto shadow-blue-200 md:w-8/12 lg:w-5/12 bg-white">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="col-span-1 lg:col-span-9">
            <div className="text-center lg:text-left">
              <h2 className="text-2xl font-bold text-zinc-700">
                Umang Khambhu
              </h2>
              <p className="mt-2 text-l font-semibold text-zinc-700">
                Studenet Coordinator
              </p>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8 text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Email</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">
                  22dit023@charusat.edu.in
                </p>
              </div>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-8  text-center items-center lg:text-left">
              <div>
                <p className="font-bold text-zinc-700">Phone</p>
              </div>

              <div>
                <p className="text-m font-semibold text-zinc-700">8758113171</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Contact;
